...
xdebug.idekey=mySecretKey666" >> /usr/local/etc/php/conf.d/docker-php-ext-xdebug.ini

# (possible useless) Give full access to www-data for mounted volumes and new files
RUN echo "umask 000" >> /root/.bashrc

EXPOSE 80 9000 9001
# todo: possible no need to use 9000

COPY ./autorun /
CMD sh /services.sh
#CMD /usr/local/bin/pydbgpproxy -d 127.0.0.1:9000 -i 127.0.0.1:9001
